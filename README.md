...
本项目提供了一个可视化的 Web 界面，方便用户进行设备管理、实时投屏和任务执行。
...
1.  **设备管理**：
    *   自动列出已连接的 USB 设备。
    *   **自动扫描 (Auto-scan)**：点击 "Scan & Connect (LAN)" 按钮，自动扫描同一局域网内开启了 ADB 调试（端口 5555）的设备并连接。
    *   支持手动输入 `IP:Port` 进行远程连接。
2.  **实时投屏**：连接设备后，可实时查看手机屏幕画面。
3.  **任务对话**：在右侧聊天框输入自然语言指令（如“打开京东搜索蓝牙耳机”），Agent 将自动执行并反馈结果。

## 配置说明

为了安全起见，建议将敏感信息（如 API Key）保存在环境变量中。

1.  复制示例配置文件：
    ```bash
    cp .env.example .env
    ```
    *(Windows 用户可手动复制或使用 `copy .env.example .env`)*
