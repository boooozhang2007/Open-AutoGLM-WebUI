...
def parse_action(response: str) -> dict[str, Any]:
    ...
    try:
        ...
        # Split by newline to handle multiple actions/garbage
        ...
        for line in lines:
            ...
            # Handle <answer> tags if they leaked
            if "<answer>" in line:
                line = line.replace("<answer>", "").replace("</answer>", "").strip()

            # Handle {do(...)} format
            if line.startswith("{") and line.endswith("}") and "do(" in line:
                line = line[1:-1].strip()

            # Handle {action="..."} format (convert to do(action="..."))
            if line.startswith("{") and line.endswith("}") and "action=" in line and "do(" not in line:
                line = "do(" + line[1:-1] + ")"
            ...
            if line.startswith("do("):
                ...
                try:
                    return {
                        "_metadata": "finish",
                        "message": line.replace("finish(message=", "")[1:-2],
                    }
                except:
                    continue

        # Fallback: try original logic on full response
